...
class HomeController extends GetxController {
  ...
  Future<void> loadData() async {
    ...
    try {
      ...
      if (hasError) {
        final errors = results.where((result) => !result.success).map((result) => result.message ?? '未知错误').join('; ');
        _error.value = errors;
      }
    } catch (e) {
  ...
  Future<void> loadComicsByCategory(String categoryId) async {
    try {
      ...
      if (result.success && result.data != null) {
        // 导航到分类页面，传递数据
        Get.toNamed('/category', arguments: {'categoryId': categoryId, 'comics': result.data});
      } else {
